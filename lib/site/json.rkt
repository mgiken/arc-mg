...
(provide write-json)
(define (write-json x [enc 'control] [o (current-output-port)])
  (define (escape m)
  ...
  (define (arc-list-denil x)
    (if (pair? x) (cons (arc-denil-car (car x)) (arc-denil-cdr (cdr x))) x))
  (define (arc-denil-car x)
    (if (eq? x 'nil) 'nil (arc-list-denil x)))
  (define (arc-denil-cdr x)
    (if (eq? x 'nil) '() (arc-list-denil x)))
  (let loop ([x x])
    (cond [(or (exact-integer? x) (inexact-real? x)) (write x o)]
          [(or (eq? x 'false) (eq? x #f))            (write-bytes #"false" o)]
          [(or (eq? x 'true)  (eq? x #t) (eq? x 't)) (write-bytes #"true"  o)]
          [(eq? x 'nil)                              (write-bytes #"[]"    o)]
          [(eq? x 'null)                             (write-bytes #"null"  o)]
          [(string? x) (write-json-string x)]
          [(symbol? x) (write-json-string (symbol->string x))]
          [(pair? x)
           (let ([x (arc-list-denil x)])
             (write-bytes #"[" o)
             (when (pair? x)
               (loop (car x))
...
(define (read-json [i (current-input-port)])
  ...
  (define (read-hash)
    ...
    (make-hash (read-list 'object #rx#"^}" read-pair)))
  ;;
  (define (read-json [top? #f])
    ...
    (cond
      ...
      [(regexp-try-match #px#"^false\\b" i) 'nil]
      [(regexp-try-match #px#"^null\\b"  i) 'nil]
      [(regexp-try-match
...
(define (jsexpr->string x [enc 'control])
  (define o (open-output-string))
  (write-json x enc o)
  (get-output-string o))
(define (jsexpr->bytes x [enc 'control])
